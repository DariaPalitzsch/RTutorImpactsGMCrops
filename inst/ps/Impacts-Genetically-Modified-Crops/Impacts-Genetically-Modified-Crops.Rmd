...
answer.quiz("Overview with dplyr")
```
...
[1]: filter() %>% group_by() %>% summarize()
[2]: select() %>% filter() %>% summarize()
[3]: select() %>% mutate() %>% group_by()
[4]: filter() %>% group_by() %>% arrange()

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Using dplyr")
```
...
[1]: year and country.
[2]: year and GM_country.
[3]: year and crop.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Average cotton yields")
```
...
[1]: The cultivation of cotton in GM regions in the experimental period increases the average yields of cotton by 0.46 tonnes/hectare compared to the pre-experimental period.
[2]: The cultivation of cotton in GM regions increases the average yields of cotton by 0.46 tonnes/hectare compared to Non-GM regions.
[3]: The cultivation of cotton in GM and Non-GM regions increases the average yields of cotton by 0.46 tonnes/hectare compared to a world where GM cultivation is banned everywhere.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("First DiD manually")
```
...
[1]: The average yield in the treatment group in the experimental period
[2]: The average yield in the control group in the pre-experimental period
[3]: The average yield in the control group
[4]: The average yield in the treatment group in the pre-experimental period

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Interpretation ß_0")
```
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Adding fixed effects")
```
...
[1]: The DiD estimator.
[2]: The significance level of the DiD estimator.
[3]: The standard error of the DiD estimator.
[4]: Nothing.

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Comparing Regression Results")
```
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Treatment values for rice")
```
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Triple DiD interaction")
```
...
```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Country fixed effects")
```
...
[1]: Country cluster-robust standard errors
[2]: Country-crop cluster-robust standard errors

```{r eval=FALSE}
# Run line to answer the quiz above
answer.quiz("Country-crop cluster-robust standard errors")
```
